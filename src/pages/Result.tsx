...
export default function Result() {
  ...
  useEffect(() => {
    ...
    if (dataParam) {
      ...
      if (decoded) { setResult(decoded); setLoaded(true); return; }
    }
    // 2. Try context result
    if (contextResult) { setResult(contextResult); setLoaded(true); return; }
    // 3. Try localStorage
    const stored = loadResultFromStorage();
    if (stored) { setResult(stored); setLoaded(true); return; }
    // 4. Redirect home
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50">
      ...
      <main className="max-w-3xl mx-auto px-4 py-10 animate-fade-in">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-6">
          <h2 className="font-bold text-slate-800 text-base mb-4">Score Breakdown</h2>
          <ScoreBreakdown percentages={result.percentages} primaryType={result.primaryType} />
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-4">
          ...
          <p className="text-sm text-slate-700 leading-relaxed">{pt.overview}</p>
          <div className="mt-4 pt-4 border-t border-slate-100">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-2">Core Motivation</p>
            <p className="text-sm text-slate-700 leading-relaxed">{pt.coreMotivation}</p>
          </div>
        </div>
        ...
        {/* ── Share actions ── */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-6">
          <h2 className="font-bold text-slate-800 text-base mb-4">Share Your Results</h2>
      ...
      <footer className="text-center text-xs text-slate-400 py-6">
        © {new Date().getFullYear()} Selfmetric · Self-reflection tool · Not a clinical diagnosis
      </footer>
    </div>
  );
}
